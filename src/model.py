...
class MPL(nn.Module):
    ...
    def __init__(self):
        super().__init__()
        self.L0 = nn.Parameter(torch.tensor(1.0, dtype=torch.float64))
        ...
        self.beta = nn.Parameter(torch.tensor(1.0, dtype=torch.float64))
        self.gamma = nn.Parameter(torch.tensor(1.0, dtype=torch.float64))
    ...
    def forward(self, data: TrainCurve):
        ...
        if step.numel() == 0:
            ...
            if max_s_val_in_batch == 0:
                ...
                k_indices = torch.arange(1, max_s_val_in_batch.item() + 1, device=device, dtype=torch.long)
                lrs_k = lrs[k_indices]
...
class LRA(nn.Module):
    ...
    def forward(self, data: TrainCurve):
        S1 = data.S1
        N = data.N
        pred = self.L0 + self.A * S1 ** (- self.alpha) + self.B * N ** (-self.beta) - self.C * N ** (-self.gamma)
        return pred
